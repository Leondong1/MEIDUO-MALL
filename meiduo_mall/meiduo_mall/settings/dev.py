...
SECRET_KEY = '7k4i1jlpr^!duior1d85(7_ou&j7s($vafz1397bf$x(t%0!hz'

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = True

# 增加可以访问后端的域名（咱们访问前后端的时候域名都是127.0.0.1）
...
# redis 配置显然为一个字典，根据Redis的不同配置，咱们可以添加不同的功能，使用不同的库就好了
CACHES = {
    "default": {
...
LOGGING = {
    ...
    'loggers': {  # 日志器  代表集成上面的信息，将咱们的日志信息均往这个对象里面去写
...
CRONJOBS = [
    # 每5分钟执行一次生成主页静态文件
    ('* * * * *', 'contents.crons.generate_static_index_html', '>> ' + os.path.join(os.path.dirname(BASE_DIR), "logs/crontab.log"))
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.8.128:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
